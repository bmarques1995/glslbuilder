...
bool GLSLBuilder::FileHandler::ReadTextFile(std::string_view path, std::string* content)
{
	bool loaded = false;
...
bool GLSLBuilder::FileHandler::WriteTextFile(std::string_view path, std::string content)
{
	bool stored = false;
...
bool GLSLBuilder::FileHandler::ReadBinFile(std::string_view path, std::byte** content, size_t* fileSize)
{
	bool loaded = false;
	std::ifstream fileStream;
	fileStream.exceptions(std::ifstream::badbit | std::ifstream::failbit);
...
bool GLSLBuilder::FileHandler::WriteBinFile(std::string_view path, std::byte* content, size_t dataSize)
{
	bool stored = false;
	std::ofstream fileStream;
	fileStream.exceptions(std::ofstream::badbit);
...
bool GLSLBuilder::FileHandler::FileExists(std::string_view path)
{
	bool loaded = false;
	std::ifstream fileStream;
