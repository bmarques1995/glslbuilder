...
std::shared_ptr<spdlog::logger> GLSLBuilderCLI::Console::s_Logger;

void GLSLBuilderCLI::Console::Init()
{
	...
	s_Logger.reset(new spdlog::logger("GLSLBuilder", client_dup_filter));
	spdlog::register_logger(s_Logger);
	s_Logger->set_level(spdlog::level::trace);
	s_Logger->flush_on(spdlog::level::trace);
}

void GLSLBuilderCLI::Console::End()
{
	s_Logger.reset();
}